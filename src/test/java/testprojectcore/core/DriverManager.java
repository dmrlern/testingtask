...
public class DriverManager {
    ...
    public void initDriver() throws Exception {
        ...
        try {
            ...
            logger.warn("Browser parameter was not passed in run command, setting the value from the config file...");
            browser = TestConfigProvider.TESTCONFIGURATION.getPropertyValue("browser");
        } finally {
            if (browser == null) {
                logger.warn("Couldn't set browser parameter from the property file either(it was null), setting the default browser...");
                browser = TestConfigProvider.WEBTESTPROPERTIES.getPropertyValue("defaultBrowser");
            }
            if (browser.equals("")) {
                logger.warn("Couldn't set browser parameter from the property file either(it was empty), setting the default browser...");
                browser = TestConfigProvider.WEBTESTPROPERTIES.getPropertyValue("defaultBrowser");
            }
        }
